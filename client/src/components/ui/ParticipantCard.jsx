...
function ParticipantCard({
  ...
  const getCardStyle = () => {
    ...
    if (participant.isAgent) {
      // Agent styling
      if (isCurrentSpeaker) {
        return `${baseClasses} border-blue-500 bg-blue-500 shadow-blue-300 shadow-xl scale-110 ` +
  ...
  const displayName = () => {
    return (
      <div className="absolute -bottom-12 left-1/2 transform -translate-x-1/2 
        ...
        <div className={`text-xs ${participant.isAgent
          ? 'text-blue-600'
  ...
  const displaySpeakingIndicator = () => {
    if (isCurrentSpeaker) {
      ...
      return (
        <div className={`absolute -top-2 -right-2 w-6 h-6 ${bgColor} rounded-full 
  ...
  return (
    ...
    >
      ...
      {/* Role Indicator */}
      <div className={`absolute -top-1 -left-1 w-4 h-4 rounded-full border-2 border-white text-xs flex items-center justify-center ${participant.isAgent
