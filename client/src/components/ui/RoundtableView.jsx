...
function RoundtableView({
  ...
  return (
    <div className="relative flex items-center justify-center min-h-[500px]">
      {/* Roundtable Surface */}
      <div
        className="relative w-96 h-96 bg-gradient-to-br from-amber-800 to-amber-900 
                      rounded-full shadow-2xl border-8 border-amber-700"
      >
        {/* Wood grain effect */}
        <div
          className="absolute inset-2 bg-gradient-to-br from-amber-700 to-amber-800 
                        rounded-full opacity-60"
        ></div>
        <div
          className="absolute inset-4 bg-gradient-to-br from-amber-600 to-amber-700 
                        rounded-full opacity-40"
        ></div>

        {/* Center Topic Area */}
        <div
          className="absolute inset-8 bg-white rounded-full shadow-inner border-4 
          ...
          {discussionStarted && currentTopic ? (
            <div className="text-center">
              ...
              {currentTopic.category && (
                <span
                  className="inline-block mt-2 px-2 py-1 bg-primary-100 text-primary-800 
        ...
        {participants.length > 0 && participants.length < 8 && (
          <>
            {Array.from(
              { length: Math.max(0, 4 - participants.length) },
              (_, index) => {
      ...
      >
        <div className="flex items-center space-x-2 text-sm">
          ...
          ></div>
          <span
            className={discussionStarted ? "text-green-700" : "text-gray-600"}
          >
            {discussionStarted
      ...
      {discussionStarted && currentSpeaker && (
        <div
          className={`absolute top-4 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-full ${
        ...
        >
          <div className="flex items-center space-x-2 text-sm">
            ...
            >
              {currentSpeaker.anonymousName || "Anonymous"} is{" "}
              {currentSpeaker.isAgent ? "facilitating" : "speaking"}
            </span>
          </div>
        </div>
      )}
