  ...
  const optimizeAudioSDP = (sessionDescription) => {
    ...
    if (opusMatch) {
      ...
      if (audioMatch) {
        ...
        sdp = sdp.replace(
          audioLineRegex,
          `m=audio ${port} UDP/TLS/RTP/SAVPF ${reorderedTypes.join(" ")}\r\n`
        );
      }
    ...
    pc.ontrack = (event) => {
      try {
        ...
        if (remoteStream) {
          ...
          const oldElement = document.getElementById(
            `remote-audio-${peerSocketId}`
          );
          if (oldElement) {
            oldElement.remove();
          }
          ...
          console.warn(
            `[Audio][Debug] No remoteStream found in ontrack for peer: ${peerSocketId}`
          );
        }
      } catch (err) {
        console.error(
          `[Audio][Debug] Error handling ontrack event for ${peerSocketId}:`,
          err
        );
      }
    };
    ...
    const handleOffer = async ({ from, sdp }) => {
      try {
        ...
        if (currentStream && userRole === "speaker") {
          ...
          console.log(
            `[Audio] Not adding tracks - Role: ${userRole}, Stream: ${!!currentStream}`
          );
        }
    ...
    const handleParticipantsUpdate = (updatedParticipants) => {
      try {
        ...
        if (localStreamRef.current && !webrtcReadySignaled) {
          ...
          console.log(
            "[Audio] Emitting ready-for-webrtc after both localStream and participants-update"
          );
          socket.emit("ready-for-webrtc");
        }

        const otherPeers = updatedParticipants.filter(
        ...
        if (userRole === "speaker" && currentStream) {
          console.log(
            `[Audio] I am a speaker, initiating connections to all peers`
          );
          otherPeers.forEach((p) => {
            ...
            if (!peers[p.socketId]) {
              ...
              pc.createOffer(offerOptions)
                ...
                .catch((err) =>
                  console.error(
                    `[Audio] Error creating/sending offer to ${p.socketId}:`,
                    err
                  )
                );
            } else {
          ...
          console.log(
            `[Audio] I am a listener, waiting for offers from speakers`
          );
        }
      } catch (err) {
        console.error(
          "[Audio] Error during participants-update handling for WebRTC:",
          err
        );
      }
    };
    ...
    const handleRoleChanged = ({ userId, newRole, participants }) => {
      try {
        ...
        if (socket.id && participants) {
          const ourParticipant = participants.find(
