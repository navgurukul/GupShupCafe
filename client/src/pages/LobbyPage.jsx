...
function LobbyPage() {
    ...
    if (
      ...
      try {
        ...
        if (sessionResult.status === "success") {
          const roomId = sessionResult.data.room_id; // Extract room_id from the room object
          sessionStorage.setItem("current_session_id", roomId);
          roomData.roomId = roomId; // Assign generated room ID

          // Create participant entry using helper function
          await createParticipantForRoom({
            userId: storedHostData.userId,
            roomId: roomId, // Use the extracted room_id
  ...
  const handleJoinSubmit = async () => {
    ...
    try {
      ...
      console.log("[Lobby][Debug] Participant created successfully");
    } catch (error) {
      console.error(
