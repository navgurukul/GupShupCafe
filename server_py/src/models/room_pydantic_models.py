...
class CreateRoomModel(BaseModel):
    ...
    room_name: Optional[str] = Field(
        None, description="Human-readable room name")

    # Room Configuration
    topic_title: str = Field(..., description="Discussion topic")
    topic_category: str = Field(
        ..., description="Topic category (Technology, Current Events, etc.)")
    max_participants: int = Field(
        default=6, description="Maximum number of participants")
    speaking_time_per_turn: int = Field(
        default=60, description="Speaking time per turn in seconds")
    num_rounds: int = Field(
        default=3, description="Number of discussion rounds")
    ...
    current_speaker_index: int = Field(
        default=0, description="Current speaker index")
    ...
    duration_seconds: int = Field(
        default=0, description="Room duration in seconds")

    # Facilitator Agent
    agent_id: Optional[str] = Field(
        None, description="AWS Strands agent instance ID")
...
class Room(BaseModel):
    ...
    max_participants: int = Field(
        default=6, description="Maximum participants")
    speaking_time: int = Field(
        default=60, description="Speaking time per turn")
    num_rounds: int = Field(default=3, description="Number of rounds")
    cefr_level: str = Field(default="A1", description="CEFR level")
    status: RoomStatus = Field(
        default=RoomStatus.WAITING, description="Room status")
    ...
    time_remaining: int = Field(
        default=60, description="Time remaining in current turn")
