...
def start_debate_tools_server(port: int = 8000, host: str = "localhost"):
    """
    ...
    """
    logger.info(f"🚀 Starting Debate Room Tools MCP Server on http://{host}:{port}")
    logger.info(f"📋 Available tools: topic_selector, get_next_speaker, validate_turn, "
               f"initialize_room, analyze_discussion_pulse")
    logger.info(f"🔗 MCP endpoint: http://{host}:{port}/mcp/")
    
    try:
        # Some MCP server versions don't accept host/port in run(); prefer uvicorn via http_app()
        # Keep this for backward-compat if available
        mcp.run(transport="streamable-http")
    except KeyboardInterrupt:
        logger.info("\n✅ Debate Room Tools MCP Server stopped gracefully")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Debate Room Tools MCP Server")
    parser.add_argument("--port", type=int, default=8000, help="Port to run server on")
    parser.add_argument("--host", type=str, default="localhost", help="Host to bind to")
    
    args = parser.parse_args()
    start_debate_tools_server(port=args.port, host=args.host)

# Expose ASGI app for uvicorn-based HTTP serving (recommended)
# Endpoint path will be /mcp/
try:
    app = mcp.http_app()
except Exception:
    app = None
